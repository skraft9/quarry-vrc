...
def upload_attachments(username, token, intent_id, file_paths):
    ...
    for fpath in file_paths:
        ...
        # The response wraps attachments in a data array.
        for att in (resp.get("data") or [resp.get("data")] if resp.get("data") else []):
            a = (att.get("attributes") or att) if isinstance(att, dict) else {}
            results.append({
                "id": att.get("id") or "",
